...
resource "aws_iam_role" "ecs_task_execution_role_master_jenkins" {
  name = "ecs_task_execution_role_master_jenkins"
...
resource "aws_iam_policy_attachment" "ecs_execution_role_policy_attachment" {
  ...
  roles      = [aws_iam_role.ecs_task_execution_role_master_jenkins.name]
...
resource "aws_iam_policy" "ecs_execution_logging_policy_master_jenkins" {
  name        = "ecs_execution_logging_policy_master_jenkins"
...
resource "aws_iam_policy_attachment" "ecs_execution_logging_attachment" {
  ...
  roles      = [aws_iam_role.ecs_task_execution_role_master_jenkins.name]
  policy_arn = aws_iam_policy.ecs_execution_logging_policy_master_jenkins.arn
}


## ecs_task_role for the task to interact with AWS services and permissions in general
resource "aws_iam_role" "ecs_task_role_master_jenkins" {
  name = "ecs_task_role_master_jenkins"
...
resource "aws_iam_policy_attachment" "ecs_task_s3_access" {
  name       = "ecs-task-s3-access"
  roles      = [aws_iam_role.ecs_task_role_master_jenkins.name]
  policy_arn = aws_iam_policy.s3_access_policy.arn
}

# Add CloudWatch logging permissions to Task Role
resource "aws_iam_policy" "ecs_task_logging_policy_master_jenkins" {
  name        = "ecs_task_logging_policy_master_jenkins"
...
resource "aws_iam_policy_attachment" "ecs_task_logging_attachment_master_jenkins" {
  name       = "ecs_task_logging_attachment_master_jenkins"
  roles      = [aws_iam_role.ecs_task_role_master_jenkins.name]
  policy_arn = aws_iam_policy.ecs_task_logging_policy_master_jenkins.arn
}
...
resource "aws_iam_role_policy_attachment" "ecs_ssm_exec_attach" {
  role       = aws_iam_role.ecs_task_role_master_jenkins.name
